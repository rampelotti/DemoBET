...
export function categorizeMarketType(type: string): MarketCategory {
  ...
  // Inclui `ALTERNATE_SPREADS_CORNERS_*` / `SPREADS_CORNERS_*` etc.
  if (type.startsWith("CORNERS_") || type.includes("SPREADS_CORNERS")) return "escanteios";
  if (type.startsWith("CARDS_") || type.includes("SPREADS_CARDS")) return "cartoes";
  if (type.includes("_H1") || type.includes("SPREADS_H1")) return "primeiro-tempo";
  if (
